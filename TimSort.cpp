...
private:
	 ...
	 */
	enum { INITIAL_TMP_STORAGE_LENGTH = 256 };

	/**
	 * Temp storage for merges.
	 */
	T *tmp; // Actual runtime type will be Object[], regardless of T
	int tmp_len;
...
public:
	 ...
	 */
	TimSort(T *a, int len) : minGallop(MIN_GALLOP), stackSize(0), tmp(NULL), runBase(NULL), runLen(NULL)
	{
		this->a = a;
		this->a_len = len;
...
private:
		...
		for ( ; start < hi; start++) {
			...
			switch(n) {
				...
				case 1:  a[left + 1] = a[left];
						 break;
				default: memcpy(a+left+1, a+left, n * sizeof(T));
			}
			a[left] = pivot;
		}
	}
	...
	void mergeLo(int base1, int len1, int base2, int len2, PR pred)
	{
		...
		int a_len = this->a_len;
		T *tmp = ensureCapacity(len1);
		memcpy(tmp, a + base1, len1 * sizeof(T));
	...
	break_outer:
		...
		if (len1 == 1) {
			...
			throw format("IllegalArgumentException: Comparison method violates its general contract!");
		} else {
			assert(len2 == 0);
			assert(len1 > 1);
			memcpy(a+dest, tmp+cursor1, len1 * sizeof(T));
	...
	void mergeHi(int base1, int len1, int base2, int len2, PR pred)
	{
		...
		memcpy(tmp, a+base2, len2 * sizeof(T));
		...
		a[dest--] = a[cursor1--];
		if (--len1 == 0) {
			memcpy(a + dest - (len2 - 1), tmp, len2 * sizeof(T));
			return;
		}
		if (len2 == 1) {
			dest -= len1;
	...
	static void rangeCheck(int arrayLen, int fromIndex, int toIndex)
	{
		...
		if (fromIndex < 0)
			throw format("ArrayIndexOutOfBoundsException(%d)", fromIndex);
		if (toIndex > arrayLen)
			throw format("ArrayIndexOutOfBoundsException(%d)", toIndex);
	}
};
